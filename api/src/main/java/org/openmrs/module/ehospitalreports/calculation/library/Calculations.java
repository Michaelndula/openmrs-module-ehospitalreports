package org.openmrs.module.ehospitalreports.calculation.library;
...
import org.openmrs.module.ehospitalreports.CoreUtils;
import org.openmrs.module.ehospitalreports.reporting.calculation.BooleanResult;
...
public class Calculations {
	...
	public static CalculationResultMap allObs(Concept concept, Collection<Integer> cohort, PatientCalculationContext context) {
		ObsForPersonDataDefinition def = new ObsForPersonDataDefinition("all obs", TimeQualifier.ANY, concept,
		        context.getNow(), null);
		return CalculationUtils.ensureEmptyListResults(
		    CalculationUtils.evaluateWithReporting(def, cohort, null, null, context), cohort);
	}
	 ...
	 * @return the obss in a calculation result map
	 */
	public static CalculationResultMap firstObsOnOrAfter(Concept concept, Date onOrAfter, Collection<Integer> cohort,
	        PatientCalculationContext context) {
		ObsForPersonDataDefinition def = new ObsForPersonDataDefinition("first obs on or after", TimeQualifier.FIRST,
		        concept, context.getNow(), onOrAfter);
		return CalculationUtils.evaluateWithReporting(def, cohort, null, null, context);
	}
	...
	/**
	 * Evaluates the last obs of a given type of each patient that occurred at least the given
	 * number of days ago
	 * 
	 * @param concept the obs' concept
	 ...
	 * @return the obss in a calculation result map
	 */
	public static CalculationResultMap lastObsOnOrBefore(Concept concept, Date onOrBefore, Collection<Integer> cohort,
	        PatientCalculationContext context) {
		// Only interested in obs before now
		onOrBefore = CoreUtils.earliest(onOrBefore, context.getNow());
		
		ObsForPersonDataDefinition def = new ObsForPersonDataDefinition("last obs on or before", TimeQualifier.LAST,
		        concept, onOrBefore, null);
		return CalculationUtils.evaluateWithReporting(def, cohort, null, null, context);
	}
	 ...
	 * @return the obss in a calculation result map
	 */
	public static CalculationResultMap lastObsAtLeastDaysAgo(Concept concept, int atLeastDaysAgo,
	 ...
	 */
	public static CalculationResultMap allEncounters(EncounterType encounterType, Collection<Integer> cohort,
	        PatientCalculationContext context) {
	 ...
	 * @return the encounters in a calculation result map
	 */
	public static CalculationResultMap lastEncounter(EncounterType encounterType, Collection<Integer> cohort,
	        PatientCalculationContext context) {
		EncountersForPatientDataDefinition def = new EncountersForPatientDataDefinition();
		if (encounterType != null) {
	 ...
	 * @return the encounters in a calculation result map
	 */
	public static CalculationResultMap allEncountersOnOrAfter(EncounterType encounterType, Date onOrAfter,
	        Collection<Integer> cohort, PatientCalculationContext context) {
		EncountersForPatientDataDefinition def = new EncountersForPatientDataDefinition();
		if (encounterType != null) {
			def.setName("all encounters of type " + encounterType.getName());
	 ...
	 */
	public static CalculationResultMap allEnrollments(Program program, Collection<Integer> cohort,
	        PatientCalculationContext context) {
		...
		CalculationResultMap results = CalculationUtils.evaluateWithReporting(def, cohort, new HashMap<String, Object>(),
		    null, context);
		return CalculationUtils.ensureEmptyListResults(results, cohort);
	}
	 ...
	 * @return the enrollments in a calculation result map
	 */
	public static CalculationResultMap firstEnrollments(Program program, Collection<Integer> cohort,
	        PatientCalculationContext context) {
		ProgramEnrollmentsForPatientDataDefinition def = new ProgramEnrollmentsForPatientDataDefinition();
